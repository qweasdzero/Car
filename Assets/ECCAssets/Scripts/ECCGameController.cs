...
namespace EndlessCarChase
{
    ...
    public class ECCGameController : MonoBehaviour
    {
        ...
        [Tooltip(
            "The layer which the cars move up and down along terrain. A 'groundObject' must not be assigned or hidden  in order to make this work")]
        public LayerMask groundLayer;

        [Tooltip(
            "The speed at which the texture of the ground object moves, make the player seems as if it is moving on the ground")]
        ...
        void LateUpdate()
        {
            if (playerObject)
            {
                if (cameraHolder)
                {
                    ...
                    if (cameraRotate > 0)
                        cameraHolder.eulerAngles = Vector3.up * Mathf.LerpAngle(cameraHolder.eulerAngles.y,
                                                       playerObject.transform.eulerAngles.y,
                                                       Time.deltaTime * cameraRotate);
                }
                ...
                if (groundObject)
                {
                    ...
                    groundObject.GetComponent<Renderer>().material.SetTextureOffset("_MainTex",
        ...
        IEnumerator GameOver(float delay)
        {
            ...
            if (gameOverCanvas)
            {
                ...
                gameOverCanvas.Find("Base/TextScore").GetComponent<Text>().text = "得分 " + score.ToString();
                ...
                if (score > highScore)
                {
                    ...
                    PlayerPrefs.SetInt(SceneManager.GetActiveScene().name + "最高分 ", score);
                }

                //Write the high sscore text
                gameOverCanvas.Find("Base/TextHighScore").GetComponent<Text>().text =
                    "最高分 " + highScore.ToString();
