...
namespace EndlessCarChase
{
    ...
    public class ECCCar : MonoBehaviour
    {
        ...
        [Tooltip(
            "A random value that is to the chase angle to make the AI cars more varied in how to chase the player")]
        ...
        private void Start()
        {
            ...
            if (Physics.Raycast(thisTransform.position + Vector3.up * 5 + thisTransform.forward * 1.0f,
                -10 * Vector3.up, out hit, 100, gameController.groundLayer)) forwardPoint = hit.point;
        ...
        void Update()
        {
            ...
            if (gameController.playerObject != this)
            {
                ...
                Ray rayRight =
                    new Ray(
                        thisTransform.position + Vector3.up * 0.2f + thisTransform.right * detectAngle * 0.5f +
                ...
                if (avoidObstacles == true && Physics.Raycast(rayRight, out hit, detectDistance) &&
                    (hit.transform.GetComponent<ECCObstacle>() ||
                     (hit.transform.GetComponent<ECCCar>() && gameController.playerObject != this)))
                {
                ...
                else if (avoidObstacles == true && Physics.Raycast(rayLeft, out hit, detectDistance) &&
                         (hit.transform.GetComponent<ECCObstacle>() ||
                          (hit.transform.GetComponent<ECCCar>() && gameController.playerObject != this))
            ...
            if (hurtDelayCount > 0 && health > 0)
            {
                ...
                if (GetComponentInChildren<MeshRenderer>())
                {
                    ...
                    {
                        if (Mathf.Round(hurtDelayCount * 10) % 2 == 0)
                        {
                            part.SetColor("_EmissionColor", Color.black);
                        }
                        else
                        {
                            part.SetColor("_EmissionColor", hurtFlashColor);
                        }
        ...
        public void Rotate(float rotateDirection)
        {
            ...
            if (rotateDirection != 0)
            {
                ...
                m_Base.localEulerAngles = new Vector3(rightAngle,
                    Mathf.LerpAngle(m_Base.localEulerAngles.y,
                        rotateDirection * driftAngle + Mathf.Sin(Time.time * 50) * hurtDelayCount * 50, Time.deltaTime),
                    0); //  Mathf.LerpAngle(thisTransform.Find("Base").localEulerAngles.y, rotateDirection * driftAngle, Time.deltaTime);
                ...
                if (chassis)
                    chassis.localEulerAngles = Vector3.forward * Mathf.LerpAngle(chassis.localEulerAngles.z,
                                                   rotateDirection * leanAngle,
                                                   Time
                                                       .deltaTime); //  Mathf.LerpAngle(thisTransform.Find("Base").localEulerAngles.y, rotateDirection * driftAngle, Time.deltaTime);
            ...
            else // Otherwise, if we are no longer rotating, straighten up the car and front wheels
            {
                ...
                m_Base.localEulerAngles = Vector3.up * Mathf.LerpAngle(thisTransform.Find("Base").localEulerAngles.y, 0,
                                              Time.deltaTime * 5);
                ...
                for (index = 0; index < wheels.Length; index++)
                {
                    ...
                    if (index < frontWheels)
                        wheels[index].localEulerAngles =
                            Vector3.up * Mathf.LerpAngle(wheels[index].localEulerAngles.y, 0, Time.deltaTime * 5);
        ...
        public void DetectGround()
        {
            ...
            thisTransform.position = new Vector3(thisTransform.position.x, groundHitInfo.point.y + 0.1f,
            ...
            if (Physics.Raycast(thisTransform.position + Vector3.up * 5 + thisTransform.forward * 1.0f,
                -10 * Vector3.up, out hit, 100, gameController.groundLayer))
            {
                forwardPoint = hit.point;
            }
            else if (gameController.groundObject && gameController.groundObject.gameObject.activeSelf == true)
            {
                forwardPoint = new Vector3(thisTransform.position.x, gameController.groundObject.position.y,
                    thisTransform.position.z);
            }
